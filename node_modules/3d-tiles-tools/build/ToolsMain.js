...
const TilesetConverter_1 = require("./tilesetProcessing/TilesetConverter");
const TilesetJsonCreator_1 = require("./tilesetProcessing/TilesetJsonCreator");
const Loggers_1 = require("./logging/Loggers");
const logger = Loggers_1.Loggers.get("CLI");
/**
...
class ToolsMain {
    ...
    static async convertB3dmToGlb(input, output, force) {
        ...
        ToolsMain.ensureCanWrite(output, force);
        const inputBuffer = fs_1.default.readFileSync(input);
        const outputBuffer = await TileFormatsMigration_1.TileFormatsMigration.convertB3dmToGlb(inputBuffer);
        fs_1.default.writeFileSync(output, outputBuffer);
    ...
    static async convertPntsToGlb(input, output, force) {
        ...
        ToolsMain.ensureCanWrite(output, force);
        const inputBuffer = fs_1.default.readFileSync(input);
        const outputBuffer = await TileFormatsMigration_1.TileFormatsMigration.convertPntsToGlb(inputBuffer);
        fs_1.default.writeFileSync(output, outputBuffer);
    ...
    static async convertI3dmToGlb(input, output, force) {
        ...
        const outputBuffer = await TileFormatsMigration_1.TileFormatsMigration.convertI3dmToGlb(inputBuffer, externalGlbResolver);
        fs_1.default.writeFileSync(output, outputBuffer);
    ...
    static async cmptToGlb(input, output, force) {
        ...
        for (let i = 0; i < glbsLength; i++) {
            ...
            const upgradedOutputBuffer = await GltfUtilities_1.GltfUtilities.upgradeGlb(glbBuffer, undefined);
    ...
    static analyzeInternal(inputBaseName, inputBuffer, outputDirectoryName, force) {
        ...
        if (magic === "b3dm" || magic === "i3dm" || magic === "pnts") {
            ...
            if (tileData.payload.length !== 0) {
                const glbJsonBuffer = GltfUtilities_1.GltfUtilities.extractJsonFromGlb(tileData.payload);
            ...
            // Handle GLB files
            let glbJsonString = "{}";
            const glbJsonBuffer = GltfUtilities_1.GltfUtilities.extractJsonFromGlb(inputBuffer);
            glbJsonString = glbJsonBuffer.toString();
            if (doFormatJson) {
                const glbJson = JSON.parse(glbJsonString);
                glbJsonString = stringify(glbJson);
    ...
    static async convert(input, inputTilesetJsonFileName, output, force) {
        ToolsMain.ensureCanWrite(output, force);
        await TilesetConverter_1.TilesetConverter.convert(input, inputTilesetJsonFileName, output, force);
    }
    ...
    static async upgrade(input, output, force, targetVersion, gltfUpgradeOptions) {
        ...
        ToolsMain.ensureCanWrite(output, force);
        await Tilesets_1.Tilesets.upgrade(input, output, force, targetVersion, gltfUpgradeOptions);
    ...
    static async createTilesetJson(inputName, output, force) {
        ...
        let contentUris = [];
        if (!Paths_1.Paths.isDirectory(inputName)) {
            ...
            const recurse = true;
            const allFiles = Iterables_1.Iterables.overFiles(inputName, recurse);
            contentUris = [...allFiles].map((fileName) => Paths_1.Paths.relativize(inputName, fileName));
        }
        logger.info("Creating tileset.json with content URIs: ", contentUris);
        const tileset = await TilesetJsonCreator_1.TilesetJsonCreator.createTilesetFromContents(baseDir, contentUris);
        const tilesetJsonString = JSON.stringify(tileset, null, 2);
        const outputDirectory = path_1.default.dirname(output);
        Paths_1.Paths.ensureDirectoryExists(outputDirectory);
