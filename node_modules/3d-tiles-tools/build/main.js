...
let logger = Loggers_1.Loggers.get("CLI");
...
function parseToolArgs(a) {
    const args = (0, yargs_1.default)(a)
        ...
        logLevel: {
            default: "info",
            description: "The log level. Valid values are 'trace', 'debug', 'info', " +
        ...
        logJson: {
            default: false,
            description: "Whether log messages should be printed as JSON instead of pretty-printing",
            global: true,
            type: "boolean",
        },
    })
        ...
        inputTilesetJsonFileName: {
            description: "The name of the tileset JSON file in the input. " +
        ...
        .command("convertB3dmToGlb", "Convert a b3dm file into a glTF asset that uses glTF extensions to " +
        "represent the batch- and feature table information", {
        i: inputStringDefinition,
        o: outputStringDefinition,
    })
        .command("convertPntsToGlb", "Convert a pnts file into a glTF asset that uses glTF extensions to " +
        "represent the point properties and batch- and feature table information", {
        i: inputStringDefinition,
        o: outputStringDefinition,
    })
        .command("convertI3dmToGlb", "Convert an i3dm file into a glTF asset that uses glTF extensions to " +
        ...
        .command("optimizeB3dm", "Pass the input b3dm through gltf-pipeline. To pass options to gltf-pipeline, " +
        ...
        .command("optimizeI3dm", "Pass the input i3dm through gltf-pipeline. To pass options to gltf-pipeline, " +
        "place them after --options. (--options -h for gltf-pipeline help)", {
        i: inputStringDefinition,
        o: outputStringDefinition,
        options: {
        ...
        .command("upgrade", "Upgrades legacy tilesets to comply to the 3D Tiles specification.\n\n" +
        ...
        .command("createTilesetJson", "Creates a 'tileset.json' file that just refers to given GLB tile content files. " +
        ...
        "recursively.", { i: inputStringDefinition, o: outputStringDefinition })
        .demandCommand(1)
        .strict();
...
async function run() {
    ...
    if (logJson === true) {
        ...
        Loggers_1.Loggers.initDefaultLogger(prettyPrint);
        logger = Loggers_1.Loggers.get("CLI");
    }
    ...
    if (logLevel !== undefined) {
        ...
        if (validLogLevels.includes(logLevel)) {
            Loggers_1.Loggers.setLevel(logLevel);
        }
        else {
            logger.warn(`Invalid log level: ${logLevel}`);
            Loggers_1.Loggers.setLevel("info");
        }
    }
...
async function runCommand(command, toolArgs, optionArgs) {
    ...
    if (command === "b3dmToGlb") {
        ...
        await ToolsMain_1.ToolsMain.convertB3dmToGlb(input, output, force);
    }
    else if (command === "convertPntsToGlb") {
        await ToolsMain_1.ToolsMain.convertPntsToGlb(input, output, force);
    }
    else if (command === "convertI3dmToGlb") {
        await ToolsMain_1.ToolsMain.convertI3dmToGlb(input, output, force);
    }
        ...
        logger.info(`The 'tilesetToDatabase' command is deprecated. Use 'convert' instead.`);
        await ToolsMain_1.ToolsMain.convert(input, undefined, output, force);
    }
    else if (command === "databaseToTileset") {
        logger.info(`The 'databaseToTileset' command is deprecated. Use 'convert' instead.`);
        await ToolsMain_1.ToolsMain.convert(input, undefined, output, force);
    }
    else if (command === "convert") {
        await ToolsMain_1.ToolsMain.convert(input, toolArgs.inputTilesetJsonFileName, output, force);
    }
    else if (command === "combine") {
        await ToolsMain_1.ToolsMain.combine(input, output, force);
    }
    else if (command === "upgrade") {
        await ToolsMain_1.ToolsMain.upgrade(input, output, force, toolArgs.targetVersion, parsedOptionArgs);
    }
        ...
        await ToolsMain_1.ToolsMain.createTilesetJson(input, output, force);
    }
    else {
