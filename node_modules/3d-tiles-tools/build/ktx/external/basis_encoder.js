...
var BASIS = (() => {
    ...
    return (function (BASIS = {}) {
        ...
        function createWasm() { var info = { "a": wasmImports }; function receiveInstance(instance, module) { var exports = instance.exports; Module["asm"] = exports; wasmMemory = Module["asm"]["W"]; updateMemoryViews(); wasmTable = Module["asm"]["_"]; addOnInit(Module["asm"]["X"]); removeRunDependency("wasm-instantiate"); return exports; } addRunDependency("wasm-instantiate"); function receiveInstantiationResult(result) { receiveInstance(result["instance"]); } if (Module["instantiateWasm"]) {
            try {
        ...
        function ___cxa_throw(ptr, type, destructor) { var info = new ExceptionInfo(ptr); info.init(type, destructor); exceptionLast = ptr; uncaughtExceptionCount++; throw exceptionLast; }
        ...
        function FS_createPreloadedFile(parent, name, url, canRead, canWrite, onload, onerror, dontCreateFile, canOwn, preFinish) { var fullname = name ? PATH_FS.resolve(PATH.join2(parent, name)) : parent; var dep = getUniqueRunDependency(`cp ${fullname}`); function processData(byteArray) { function finish(byteArray) { if (preFinish)
        ...
        function __embind_finalize_value_object(structType) { var reg = structRegistrations[structType]; delete structRegistrations[structType]; var rawConstructor = reg.rawConstructor; var rawDestructor = reg.rawDestructor; var fieldRecords = reg.fields; var fieldTypes = fieldRecords.map(field => field.getterReturnType).concat(fieldRecords.map(field => field.setterArgumentType)); whenDependentTypesAreResolved([structType], fieldTypes, fieldTypes => { var fields = {}; fieldRecords.forEach((field, i) => { var fieldName = field.fieldName; var getterReturnType = fieldTypes[i]; var getter = field.getter; var getterContext = field.getterContext; var setterArgumentType = fieldTypes[i + fieldRecords.length]; var setter = field.setter; var setterContext = field.setterContext; fields[fieldName] = { read: ptr => { return getterReturnType["fromWireType"](getter(getterContext, ptr)); }, write: (ptr, o) => { var destructors = []; setter(setterContext, ptr, setterArgumentType["toWireType"](destructors, o)); runDestructors(destructors); } }; }); return [{ name: reg.name, "fromWireType": function (ptr) { var rv = {}; for (var i in fields) {
        ...
        function __emval_as(handle, returnType, destructorsRef) { handle = Emval.toValue(handle); returnType = requireRegisteredType(returnType, "emval::as"); var destructors = []; var rd = Emval.toHandle(destructors); HEAPU32[destructorsRef >> 2] = rd; return returnType["toWireType"](destructors, handle); }
        var emval_symbols = {};
        ...
        function __emval_get_module_property(name) { name = getStringOrSymbol(name); return Emval.toHandle(Module[name]); }
        ...
        function craftEmvalAllocator(argCount) { var argsList = ""; for (var i = 0; i < argCount; ++i) {
            argsList += (i !== 0 ? ", " : "") + "arg" + i;
        } var getMemory = () => HEAPU32; var functionBody = "return function emval_allocator_" + argCount + "(constructor, argTypes, args) {\n" + "  var HEAPU32 = getMemory();\n"; for (var i = 0; i < argCount; ++i) {
        ...
        var emval_newers = {};
        function __emval_new(handle, argCount, argTypes, args) { handle = Emval.toValue(handle); var newer = emval_newers[argCount]; if (!newer) {
        ...
        init_emval();
        var wasmImports = { "z": ___cxa_throw, "D": ___syscall_fcntl64, "M": ___syscall_ioctl, "N": ___syscall_openat, "u": __embind_finalize_value_object, "H": __embind_register_bigint, "R": __embind_register_bool, "x": __embind_register_class, "w": __embind_register_class_constructor, "a": __embind_register_class_function, "e": __embind_register_constant, "Q": __embind_register_emval, "o": __embind_register_enum, "c": __embind_register_enum_value, "F": __embind_register_float, "k": __embind_register_function, "l": __embind_register_integer, "g": __embind_register_memory_view, "E": __embind_register_std_string, "y": __embind_register_std_wstring, "v": __embind_register_value_object, "d": __embind_register_value_object_field, "S": __embind_register_void, "I": __emscripten_throw_longjmp, "n": __emval_as, "p": __emval_call_void_method, "b": __emval_decref, "A": __emval_get_global, "q": __emval_get_method_caller, "s": __emval_get_module_property, "h": __emval_get_property, "j": __emval_incref, "r": __emval_new, "i": __emval_new_cstring, "m": __emval_run_destructors, "f": _abort, "O": _emscripten_date_now, "J": _emscripten_get_heap_max, "P": _emscripten_memcpy_big, "K": _emscripten_resize_heap, "B": _fd_close, "L": _fd_read, "G": _fd_seek, "C": _fd_write, "U": invoke_ii, "t": invoke_vi, "T": invoke_vii, "V": invoke_viii };
        var asm = createWasm();
        var ___wasm_call_ctors = function () { return (___wasm_call_ctors = Module["asm"]["X"]).apply(null, arguments); };
        var _free = function () { return (_free = Module["asm"]["Y"]).apply(null, arguments); };
        ...
        var stackSave = function () { return (stackSave = Module["asm"]["da"]).apply(null, arguments); };
        var stackRestore = function () { return (stackRestore = Module["asm"]["ea"]).apply(null, arguments); };
        var ___cxa_is_pointer_type = function () { return (___cxa_is_pointer_type = Module["asm"]["fa"]).apply(null, arguments); };
        var dynCall_jiiii = Module["dynCall_jiiii"] = function () { return (dynCall_jiiii = Module["dynCall_jiiii"] = Module["asm"]["ga"]).apply(null, arguments); };
        var dynCall_iij = Module["dynCall_iij"] = function () { return (dynCall_iij = Module["dynCall_iij"] = Module["asm"]["ha"]).apply(null, arguments); };
        var dynCall_ji = Module["dynCall_ji"] = function () { return (dynCall_ji = Module["dynCall_ji"] = Module["asm"]["ia"]).apply(null, arguments); };
