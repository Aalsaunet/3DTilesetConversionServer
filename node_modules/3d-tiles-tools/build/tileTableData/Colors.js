...
class Colors {
     ...
     */
    static standardRGBToNormalizedLinearRGBA(input) {
        const normalizedStandardRGB = input.map((b) => b / 255.0);
        const normalizedStandardRGBA = [...normalizedStandardRGB, 1.0];
        const normalizedLinearRGBA = Colors.normalizedStandardRGBAToNormalizedLinearRGBA(normalizedStandardRGBA);
     ...
     * @returns The four linear RGBA components
     */
    static standardRGBAToNormalizedLinearRGBA(input) {
     ...
     * @returns The four linear RGBA components
     */
    static standardRGB565ToNormalizedLinearRGBA(input) {
        const normalizedStandardRGBA = Colors.decodeStandardRGB565ToNormalizedStandardRGBA(input);
        const normalizedLinearRGBA = Colors.normalizedStandardRGBAToNormalizedLinearRGBA(normalizedStandardRGBA);
        return normalizedLinearRGBA;
    }
    // From CesiumJS AttributeCompression
    static normalizedStandardRGBAToNormalizedLinearRGBA(srgb) {
        const linear = Array(4);
        linear[3] = srgb[3];
        for (let i = 0; i < 3; i++) {
            const c = srgb[i];
            if (c <= 0.04045) {
     ...
     */
    static decodeStandardRGB565ToNormalizedStandardRGBA(value) {
        const mask5 = (1 << 5) - 1;
        const mask6 = (1 << 6) - 1;
