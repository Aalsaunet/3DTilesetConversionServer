...
exports.TileTableData = void 0;
const Iterables_1 = require("../base/Iterables");
const PropertyModels_1 = require("../metadata/PropertyModels");
const NumericBuffers_1 = require("../metadata/binary/NumericBuffers");
const NumericPropertyModel_1 = require("../metadata/binary/NumericPropertyModel");
...
class TileTableData {
     ...
     */
    static createPositions(featureTable, binary, numPositions) {
        if (featureTable.POSITION) {
            const byteOffset = featureTable.POSITION.byteOffset;
            return TileTableData.createPositionsFromBinary(binary, byteOffset, numPositions);
        }
        if (featureTable.POSITION_QUANTIZED) {
            const quantization = TileTableData.obtainQuantizationOffsetScale(featureTable, binary);
            if (!quantization) {
                throw new TileFormatError_1.TileFormatError(`The feature table contains POSITION_QUANTIZED, but not ` +
                    `QUANTIZED_VOLUME_OFFSET and QUANTIZED_VOLUME_OFFSET`);
            }
            const byteOffset = featureTable.POSITION_QUANTIZED.byteOffset;
            const quantizedPositions = TileTableData.createQuantizedPositionsFromBinary(binary, byteOffset, numPositions);
            // The 'quantization.offset' will become part of the 'global position'
            // so use an offset of [0,0,0] here
            const offset = [0, 0, 0];
            const dequantization = TileTableData.createDequantization(offset, quantization.scale);
            return Iterables_1.Iterables.map(quantizedPositions, dequantization);
        }
        throw new TileFormatError_1.TileFormatError("The feature table contains neither POSITION nor POSITION_QUANTIZED");
    }
    /**
     ...
     * @returns The the iterable over the data
     */
    static createPositionsFromBinary(binary, byteOffset, numPositions) {
        const legacyType = "VEC3";
        const legacyComponentType = "FLOAT";
        return TileTableData.createNumericArrayIterable(legacyType, legacyComponentType, binary, byteOffset, numPositions);
    }
    /**
     ...
     * @returns The the iterable over the data
     */
    static createQuantizedPositionsFromBinary(binary, byteOffset, numPositions) {
        const legacyType = "VEC3";
     ...
     */
    static obtainQuantizationOffsetScale(featureTable, featureTableBinary) {
        ...
        const volumeOffset = TileTableData.obtainNumberArray(featureTableBinary, featureTable.QUANTIZED_VOLUME_OFFSET, 3, "FLOAT32");
        const volumeScale = TileTableData.obtainNumberArray(featureTableBinary, featureTable.QUANTIZED_VOLUME_SCALE, 3, "FLOAT32");
        return {
            offset: volumeOffset,
            scale: volumeScale,
        };
     ...
     */
    static obtainRtcCenter(rtcCenter, binary) {
     ...
     */
    static obtainGlobalPosition(featureTable, featureTableBinary) {
        ...
        if (featureTable.RTC_CENTER) {
            rtcCenter = TileTableData.obtainRtcCenter(featureTable.RTC_CENTER, featureTableBinary);
            globalPosition = [rtcCenter[0], rtcCenter[1], rtcCenter[2]];
        }
        // Add the quantization offset to the global position
        const quantization = TileTableData.obtainQuantizationOffsetScale(featureTable, featureTableBinary);
        if (quantization) {
            if (!globalPosition) {
     ...
     */
    static createBatchIdsFromBinary(binary, byteOffset, legacyComponentType, numPoints) {
        const batchIds = TileTableData.createNumericScalarIterable("SCALAR", legacyComponentType, binary, byteOffset, numPoints);
        return batchIds;
    }
    /**
     ...
     */
    static obtainNumberArray(binary, property, length, componentType) {
        if (TileTableData.isBinaryBodyOffset(property)) {
            const byteOffset = property.byteOffset;
            const resultValues = NumericBuffers_1.NumericBuffers.getNumericArrayFromBuffer(binary, byteOffset, length, componentType);
     ...
     */
    static isBinaryBodyOffset(p) {
        return p.byteOffset !== undefined;
    }
    /**
     ...
     */
    static isBatchTableBinaryBodyReference(p) {
        const r = p;
        return (r.byteOffset !== undefined &&
     ...
     */
    static createNumericArrayIterable(legacyType, legacyComponentType, binary, byteOffset, numElements) {
        const propertyModel = TileTableData.createNumericPropertyModel(legacyType, legacyComponentType, binary, byteOffset);
        return PropertyModels_1.PropertyModels.createNumericArrayIterable(propertyModel, numElements);
    }
    /**
     * Creates an iterable over the values that are stored in the
     * given batch- or feature table data, assuming that they are
     ...
     * @returns The iterable
     */
    static createNumericScalarIterable(legacyType, legacyComponentType, binary, byteOffset, numElements) {
        const propertyModel = TileTableData.createNumericPropertyModel(legacyType, legacyComponentType, binary, byteOffset);
        return PropertyModels_1.PropertyModels.createNumericScalarIterable(propertyModel, numElements);
    }
    /**
     ...
     */
    static createNumericPropertyModel(legacyType, legacyComponentType, binary, byteOffset) {
        const type = TileTableData.convertLegacyTypeToType(legacyType);
        const componentType = TileTableData.convertLegacyComponentTypeToComponentType(legacyComponentType);
        const valuesBuffer = binary.subarray(byteOffset);
        const propertyModel = new NumericPropertyModel_1.NumericPropertyModel(type, valuesBuffer, componentType);
     ...
     */
    static createDequantization(volumeOffset, volumeScale) {
     ...
     */
    static obtainBatchIdComponentType(featureTable) {
     ...
     */
    static convertLegacyTypeToType(legacyType) {
        switch (legacyType) {
            case "SCALAR":
            case "VEC2":
            case "VEC3":
        ...
        throw new TileFormatError_1.TileFormatError(`Table contains property with unknown type ${legacyType}`);
    }
    /**
     ...
     * @throws TileFormatError If the input type is invalid
     */
    static convertLegacyComponentTypeToComponentType(legacyComponentType) {
        ...
        throw new TileFormatError_1.TileFormatError(`Table contains property with unknown component type ${legacyComponentType}`);
    }
}
exports.TileTableData = TileTableData;
