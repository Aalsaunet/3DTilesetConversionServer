...
class TilesetUpgrader {
    ...
    constructor(targetVersion, gltfUpgradeOptions) {
        ...
        this.processEntryB3dm = async (sourceEntry) => {
            ...
            if (this.upgradeOptions.upgradeB3dmToGlb) {
                ...
                targetKey = this.processContentUri(sourceKey);
                targetValue = await TileFormatsMigration_1.TileFormatsMigration.convertB3dmToGlb(sourceValue);
            }
            else if (this.upgradeOptions.upgradeB3dmGltf1ToGltf2) {
                logger.debug(`  Upgrading GLB in ${sourceKey}`);
                targetValue = await ContentUpgrades_1.ContentUpgrades.upgradeB3dmGltf1ToGltf2(sourceValue, this.gltfUpgradeOptions);
            }
            else {
                logger.debug(`  Not upgrading ${sourceKey} (disabled via option)`);
            }
        ...
        this.processEntryI3dm = async (sourceEntry) => {
            ...
            if (this.upgradeOptions.upgradeI3dmToGlb) {
                ...
                targetValue = await TileFormatsMigration_1.TileFormatsMigration.convertI3dmToGlb(sourceValue, externalGlbResolver);
            }
            else if (this.upgradeOptions.upgradeI3dmGltf1ToGltf2) {
                logger.debug(`  Upgrading GLB in ${sourceKey}`);
                targetValue = await ContentUpgrades_1.ContentUpgrades.upgradeI3dmGltf1ToGltf2(sourceValue, this.gltfUpgradeOptions);
            }
            else {
                logger.debug(`  Not upgrading ${sourceKey} (disabled via option)`);
            }
    ...
    static optionsFor(version) {
        ...
        throw new DeveloperError_1.DeveloperError(`Invalid target version ${version} - ` +
