...
const PntsPointClouds_1 = require("../contentProcessing/pointClouds/PntsPointClouds");
...
const TileTableDataI3dm_1 = require("../tileTableData/TileTableDataI3dm");
const VecMath_1 = require("../tileTableData/VecMath");
const Loggers_1 = require("../logging/Loggers");
const logger = Loggers_1.Loggers.get("tilesetProcessing");
...
class TilesetJsonCreator {
    ...
    static async createLeafTileFromContent(baseDir, contentUri) {
        ...
        const boundingBox = await TilesetJsonCreator.computeContentDataBoundingBox(contentUri, data, externalGlbResolver);
        if (!boundingBox) {
            logger.warn(`Content data type of ${contentUri} is not supported.`);
            return undefined;
        }
        logger.debug(`Bounding box for ${contentUri} is ` +
            `${boundingBox.min} ... ${boundingBox.max}`);
        const boundingVolumeBox = BoundingVolumes_1.BoundingVolumes.createBoundingVolumeBoxFromGltfBoundingBox(boundingBox);
     ...
     * resolved
     */
    static async computeContentDataBoundingBox(contentUri, data, externalGlbResolver) {
        const contentData = new BufferedContentData_1.BufferedContentData(contentUri, data);
        const contentDataType = await ContentDataTypeRegistry_1.ContentDataTypeRegistry.findContentDataType(contentData);
        if (contentDataType === ContentDataTypes_1.ContentDataTypes.CONTENT_TYPE_GLB) {
            return TilesetJsonCreator.computeBoundingBoxFromGlb(data);
        }
        else if (contentDataType === ContentDataTypes_1.ContentDataTypes.CONTENT_TYPE_PNTS) {
            return TilesetJsonCreator.computeBoundingBoxFromPnts(data);
        }
        else if (contentDataType === ContentDataTypes_1.ContentDataTypes.CONTENT_TYPE_B3DM) {
            return TilesetJsonCreator.computeBoundingBoxFromB3dm(data);
        }
        else if (contentDataType === ContentDataTypes_1.ContentDataTypes.CONTENT_TYPE_I3DM) {
            return TilesetJsonCreator.computeBoundingBoxFromI3dm(data, externalGlbResolver);
        }
        else if (contentDataType === ContentDataTypes_1.ContentDataTypes.CONTENT_TYPE_CMPT) {
            return TilesetJsonCreator.computeBoundingBoxFromCmpt(data, externalGlbResolver);
        }
        return undefined;
    }
    /**
    ...
    static createParentTile(children) {
        ...
        const parentBoundingVolumeBox = TilesetJsonCreator.computeTilesUnionBoundingVolumeBox(children);
        const tile = {
            boundingVolume: {
     ...
     */
    static computeTilesUnionBoundingVolumeBox(tiles) {
        const boundingVolumeBoxes = tiles
            .map((t) => t.boundingVolume.box)
            .filter((b) => b !== undefined);
        return TilesetJsonCreator.computeUnionBoundingVolumeBox(boundingVolumeBoxes);
    }
    /**
     ...
     */
    static computeUnionBoundingVolumeBox(boundingVolumeBoxes) {
        let unionBoundingBox = undefined;
        for (const boundingVolumeBox of boundingVolumeBoxes) {
            const boundingBox = BoundingVolumes_1.BoundingVolumes.createBoundingBoxForBoundingVolumeBox(boundingVolumeBox);
            if (!unionBoundingBox) {
                unionBoundingBox = boundingBox;
            }
            else {
                unionBoundingBox = BoundingVolumes_1.BoundingVolumes.computeBoundingBoxUnion(unionBoundingBox, boundingBox);
            }
        }
        if (!unionBoundingBox) {
            unionBoundingBox = BoundingVolumes_1.BoundingVolumes.createUnitCubeBoundingBox();
        }
        logger.debug(`Union bounding box is ` +
            `${unionBoundingBox.min} ... ${unionBoundingBox.max}`);
        return BoundingVolumes_1.BoundingVolumes.createBoundingVolumeBoxFromBoundingBox(unionBoundingBox);
    }
    /**
    ...
    static async computeBoundingBoxFromPnts(pntsBuffer) {
        ...
        const tileData = TileFormats_1.TileFormats.readTileData(pntsBuffer);
        ...
        for (const localPosition of localPositions) {
            ...
            BoundingVolumes_1.BoundingVolumes.min(min, position, min);
            BoundingVolumes_1.BoundingVolumes.max(max, position, max);
        }
        const result = {
            min: min,
            max: max,
     ...
     * @returns A promise to the bounding box
     */
    static async computeBoundingBoxFromB3dm(b3dmBuffer) {
        ...
        const gltfBoundngBox = await TilesetJsonCreator.computeBoundingBoxFromGlb(glbBuffer);
        ...
        if (featureTable.RTC_CENTER) {
            ...
            const b3dmBoundingBox = BoundingVolumes_1.BoundingVolumes.translateBoundingBox(gltfBoundngBox, [tx, ty, tz]);
     ...
     * resolved
     */
    static async computeBoundingBoxFromI3dm(i3dmBuffer, externalGlbResolver) {
        ...
        const tileData = TileFormats_1.TileFormats.readTileData(i3dmBuffer);
        ...
        // Compute the bounding box from the payload (GLB data)
        const gltfBoundingBox = await TilesetJsonCreator.computeBoundingBoxFromGlb(glbBuffer);
        const gltfCorners = BoundingVolumes_1.BoundingVolumes.computeBoundingBoxCorners(gltfBoundingBox);
        // Compute the instance matrices of the I3DM data
        const featureTable = tileData.featureTable.json;
        const featureTableBinary = tileData.featureTable.binary;
        const numInstances = featureTable.INSTANCES_LENGTH;
        const instanceMatrices = TileTableDataI3dm_1.TileTableDataI3dm.createInstanceMatrices(featureTable, featureTableBinary, numInstances);
        ...
        const matrixZupToYup = VecMath_1.VecMath.createZupToYupPacked4();
        const matrixYupToZup = VecMath_1.VecMath.createYupToZupPacked4();
        const min = [Infinity, Infinity, Infinity];
        const max = [-Infinity, -Infinity, -Infinity];
        for (const matrix of instanceMatrices) {
            ...
            const gltfMatrix = VecMath_1.VecMath.multiplyAll4([
                matrixZupToYup,
                matrix,
                matrixYupToZup,
            ]);
            for (const gltfCorner of gltfCorners) {
                const transformedCorner = TilesetJsonCreator.transformPoint3D(gltfMatrix, gltfCorner);
                BoundingVolumes_1.BoundingVolumes.min(min, transformedCorner, min);
                BoundingVolumes_1.BoundingVolumes.max(max, transformedCorner, max);
            }
        }
        return {
            min: min,
     ...
     * @returns A promise to the bounding box
     */
    static async computeBoundingBoxFromCmpt(cmptBuffer, externalGlbResolver) {
        let unionBoundingBox = undefined;
        const compositeTileData = TileFormats_1.TileFormats.readCompositeTileData(cmptBuffer);
        const buffers = compositeTileData.innerTileBuffers;
        for (const buffer of buffers) {
            const innerBoundingBox = await TilesetJsonCreator.computeContentDataBoundingBox("[inner tile of CMPT]", buffer, externalGlbResolver);
            if (!innerBoundingBox) {
                continue;
            }
            logger.debug(`  Inner bounding box for CMPT is ` +
                `${innerBoundingBox.min} ... ${innerBoundingBox.max}`);
            if (!unionBoundingBox) {
                unionBoundingBox = innerBoundingBox;
            }
            else {
                unionBoundingBox = BoundingVolumes_1.BoundingVolumes.computeBoundingBoxUnion(unionBoundingBox, innerBoundingBox);
            }
        }
        if (!unionBoundingBox) {
            unionBoundingBox = BoundingVolumes_1.BoundingVolumes.createUnitCubeBoundingBox();
     ...
     * @returns A promise to the bounding box
     */
    static async computeBoundingBoxFromGlb(glbBuffer) {
        ...
        if (scene) {
            const bounds = (0, core_1.getBounds)(scene);
            return bounds;
        }
        logger.warn("No scenes found in glTF - using unit bounding box");
        return BoundingVolumes_1.BoundingVolumes.createUnitCubeBoundingBox();
    }
}
