...
exports.TilesetObjectUpgrader = void 0;
...
const logger = Loggers_1.Loggers.get("upgrade");
/**
 ...
 * @internal
 */
class TilesetObjectUpgrader {
    /**
     * Creates a new instance
     *
     * @param upgradeOptions - The `UpgradeOptions`
     */
    constructor(upgradeOptions) {
     ...
     */
    async upgradeTilesetObject(tileset) {
        if (this.upgradeOptions.upgradedAssetVersionNumber) {
            logger.debug(`Upgrading asset version number`);
            this.upgradeAssetVersionNumber(tileset, this.upgradeOptions.upgradedAssetVersionNumber);
        }
     ...
     */
    upgradeAssetVersionNumber(tileset, targetVersion) {
        if (tileset.asset.version !== targetVersion) {
            logger.trace(`  Upgrading asset version from ${tileset.asset.version} to ${targetVersion}`);
    ...
    upgradeContentUrlToUri(content) {
        ...
        if (legacyContent.url) {
            logger.trace(`  Renaming 'url' property for content ${legacyContent.url} to 'uri'`);
            content.uri = legacyContent.url;
            delete legacyContent.url;
            return;
        }
        // This should never be the case:
        logger.warn("  The content does not have a 'uri' property (and no legacy 'url' property)");
    }
    /**
     ...
     * @param tileset - The tileset
     */
    async upgradeEmptyChildrenToUndefined(tileset) {
        const root = tileset.root;
        await Tiles_1.Tiles.traverseExplicit(root, async (tilePath) => {
            const tile = tilePath[tilePath.length - 1];
    ...
    async upgradeRefineValues(tileset) {
        ...
        await Tiles_1.Tiles.traverseExplicit(root, async (tilePath) => {
            ...
            if (tile.refine && tile.refine !== "ADD" && tile.refine !== "REPLACE") {
                ...
                logger.trace(`  Renaming 'refine' value from ${oldValue} to ${newValue}`);
...
exports.TilesetObjectUpgrader = TilesetObjectUpgrader;
