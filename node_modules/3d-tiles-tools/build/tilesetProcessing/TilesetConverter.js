...
class TilesetConverter {
    ...
    static async convert(input, inputTilesetJsonFileName, output, force) {
        if (inputTilesetJsonFileName === undefined) {
            inputTilesetJsonFileName = Tilesets_1.Tilesets.determineTilesetJsonFileName(input);
        }
        const outputTilesetJsonFileName = Tilesets_1.Tilesets.determineTilesetJsonFileName(output);
        const inputExtension = path_1.default.extname(input).toLowerCase();
        if (inputExtension === ".zip") {
            await ZipToPackage_1.ZipToPackage.convert(input, inputTilesetJsonFileName, output, force);
            return;
        }
            ...
            if (content) {
                if (key === inputTilesetJsonFileName) {
                    ...
                    if (inputTilesetJsonFileName !== outputTilesetJsonFileName) {
                        logger.debug(`Storing ${inputTilesetJsonFileName} from ${input} ` +
        ...
        if (!inputTilesetJsonFileNameWasFound) {
            throw new TilesetError_1.TilesetError(`File ${inputTilesetJsonFileName} was not found in ${input}`);
        }
        if (causedDuplicate) {
            throw new TilesetError_1.TilesetError(`The input tileset JSON file name ${inputTilesetJsonFileName} ` +
