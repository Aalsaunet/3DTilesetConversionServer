...
class S2Cell {
     ...
     * @internal
     */
    static countTrailingZeroBits(x) {
    ...
    static fromFacePositionLevel(face, position, level) {
        if (face < 0 || face > 5) {
            throw new DeveloperError_1.DeveloperError("Invalid S2 Face (must be within 0-5)");
        }
        if (level < 0 || level > S2Cell.S2_MAX_LEVEL) {
            throw new DeveloperError_1.DeveloperError("Invalid level (must be within 0-30)");
        }
        if (position < 0 || position >= Math.pow(4, level)) {
            throw new DeveloperError_1.DeveloperError("Invalid Hilbert position for level");
        }
        ...
        const positionSuffixPadding = Array(S2Cell.S2_POSITION_BITS - 2 * level).join("0");
...
S2Cell.getTokenFromId = function (cellId) {
    const trailingZeroHexChars = Math.floor(S2Cell.countTrailingZeroBits(cellId) / 4);
