...
class TileFormatsMigrationI3dm {
    ...
    static async convertI3dmToGlb(i3dmBuffer, externalGlbResolver) {
        ...
        if (tileData.header.gltfFormat === 1) {
            ...
            if (!glbBuffer) {
                throw new TileFormatError_1.TileFormatError(`Could not resolve external GLB from ${glbUri}`);
            }
        }
        ...
        const worldPositions = TileTableDataI3dm_1.TileTableDataI3dm.createWorldPositions(featureTable, featureTableBinary, numInstances);
        ...
        const positionsCenter = VecMath_1.VecMath.computeMean3D(worldPositions);
        // Compute the translations, relative to the center,
        const translations = Iterables_1.Iterables.map(worldPositions, (p) => {
            return VecMath_1.VecMath.subtract(p, positionsCenter);
        });
        ...
        const rotationQuaternions = TileTableDataI3dm_1.TileTableDataI3dm.createRotationQuaternions(featureTable, featureTableBinary, numInstances);
        const scales3D = TileTableDataI3dm_1.TileTableDataI3dm.createScales3D(featureTable, featureTableBinary, numInstances);
        const i3dmMatrices = TileTableDataI3dm_1.TileTableDataI3dm.createMatrices(translations, rotationQuaternions, scales3D, numInstances);
        ...
        for (const i3dmMatrix of i3dmMatrices) {
            ...
            const trs = VecMath_1.VecMath.decomposeMatrixTRS(gltfMatrix);
        ...
        if (batchId) {
            batchIds = TileTableDataI3dm_1.TileTableDataI3dm.createBatchIds(featureTable, featureTableBinary);
        ...
        if (batchIds) {
            extInstanceFeatures = document.createExtension(EXTInstanceFeatures_1.EXTInstanceFeatures);
            ...
            if (featureIdComponentType === "UINT8") {
                ...
                throw new TileFormatError_1.TileFormatError(`Expected UINT8, UINT16 or UINT32 as the ` +
