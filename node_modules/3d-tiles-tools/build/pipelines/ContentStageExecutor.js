...
const logger = Loggers_1.Loggers.get("pipeline");
/**
...
class ContentStageExecutor {
    ...
    static async executeContentStageInternal(contentStage, tilesetProcessor) {
        ...
        else if (contentStage.name === ContentStages_1.ContentStages.CONTENT_STAGE_CONVERT_B3DM_TO_GLB) {
            await ContentStageExecutor.executeConvertB3dmToGlb(tilesetProcessor);
        }
        else if (contentStage.name === ContentStages_1.ContentStages.CONTENT_STAGE_CONVERT_PNTS_TO_GLB) {
     ...
     * an error.
     */
    static async executeConvertB3dmToGlb(tilesetProcessor) {
        // Define the rule for updating the key (file name) of
        // the entries, as well as possible template URIs of
        // implicit tileset roots.
        const uriProcessor = (uri) => {
        ...
        const entryProcessor = async (sourceEntry, type) => {
            ...
            const targetValue = await TileFormatsMigration_1.TileFormatsMigration.convertB3dmToGlb(sourceEntry.value);
            const targetEntry = {
                key: uriProcessor(sourceEntry.key),
                value: targetValue,
            };
     ...
     * an error.
     */
    static async executeConvertPntsToGlb(tilesetProcessor) {
        // Define the rule for updating the key (file name) of
        // the entries, as well as possible template URIs of
        // implicit tileset roots.
        const uriProcessor = (uri) => {
            if (Paths_1.Paths.hasExtension(uri, ".pnts")) {
                return Paths_1.Paths.replaceExtension(uri, ".glb");
            }
            return uri;
        };
        ...
        const entryProcessor = async (sourceEntry, type) => {
            if (type !== ContentDataTypes_1.ContentDataTypes.CONTENT_TYPE_PNTS) {
                return sourceEntry;
            }
            const targetValue = await TileFormatsMigration_1.TileFormatsMigration.convertPntsToGlb(sourceEntry.value);
            const targetEntry = {
                key: uriProcessor(sourceEntry.key),
                value: targetValue,
            };
    ...
    static async executeOptimizeGlb(tilesetProcessor, options) {
        ...
        const entryProcessor = async (sourceEntry, type) => {
            ...
            const targetValue = await GltfUtilities_1.GltfUtilities.optimizeGlb(sourceEntry.value, options);
            const targetEntry = {
                key: sourceEntry.key,
                value: targetValue,
            };
