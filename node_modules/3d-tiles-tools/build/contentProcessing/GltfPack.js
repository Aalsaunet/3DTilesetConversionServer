...
exports.GltfPack = void 0;
const gltfpack_1 = __importDefault(require("gltfpack"));
...
class GltfPack {
     ...
     */
    static async process(inputGlb, options) {
        ...
        const io = {
            read: function (path) {
                ...
                throw new ContentError_1.ContentError(`The GLB data has an external reference to ${path}. ` +
                    `External references are not supported.`);
            },
            write: function (path, data) {
                if (path === outputName) {
                    outputGlb = Buffer.from(data.buffer.slice(data.byteOffset, data.byteLength + data.byteOffset));
                }
                else {
                    throw new ContentError_1.ContentError(`gltfpack unexpectedly tried to write to ${path}. ` +
                        `External references are not supported.`);
                }
            },
        };
        ...
        await gltfpack_1.default.pack(args, io);
        return outputGlb;
    }
    /**
     ...
     */
    static createCommandLineArguments(options) {
        const args = [];
        if (options.c === true) {
            args.push("-c");
        }
