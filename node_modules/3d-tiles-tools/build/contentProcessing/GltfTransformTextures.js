...
class GltfTransformTextures {
     ...
     */
    static createTransformTexturesToKtx(etc1sOptions, uastcOptions) {
        return async (document) => {
            await GltfTransformTextures.encodeTexturesToKtx(document, etc1sOptions, uastcOptions);
        };
    }
    /**
     ...
     * @param uastcOptions - The options for UASTC compression
     */
    static async encodeTexturesToKtx(document, etc1sOptions, uastcOptions) {
        const root = document.getRoot();
        const textures = root.listTextures();
        let addExtension = false;
        for (const texture of textures) {
            const didEncode = await GltfTransformTextures.encodeTextureToKtx(texture, etc1sOptions, uastcOptions);
            if (didEncode) {
                addExtension = true;
            }
        }
        if (addExtension) {
            const khrTextureBasisuExtension = document.createExtension(extensions_1.KHRTextureBasisu);
     ...
     */
    static async encodeTextureToKtx(texture, etc1sOptions, uastcOptions) {
        ...
        const outputImageData = await KtxUtility_1.KtxUtility.convertImageData(inputImageData, options);
        ...
        if (oldUri) {
            const extension = path_1.default.extname(oldUri);
     ...
     */
    static useUastc(texture) {
        const slots = (0, functions_1.listTextureSlots)(texture);
        const uastcSlots = [
            "normalTexture",
            "occlusionTexture",
     ...
     */
    static includesAny(included, including) {
...
exports.GltfTransformTextures = GltfTransformTextures;
