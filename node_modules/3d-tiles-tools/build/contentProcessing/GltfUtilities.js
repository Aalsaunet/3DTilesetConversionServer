...
class GltfUtilities {
     ...
     * for a glTF header
     */
    static getGltfVersion(glbBuffer) {
     ...
     */
    static extractDataFromGlb(glbBuffer) {
     ...
     * @throws TileFormatError If the input does not contain valid GLB data.
     */
    static extractDataFromGlb1(glbBuffer) {
     ...
     * @throws TileFormatError If the input does not contain valid GLB data.
     */
    static extractDataFromGlb2(glbBuffer) {
        if (glbBuffer.length < 20) {
            throw new TileFormatError_1.TileFormatError(`Expected at least 20 bytes, but only got ${glbBuffer.length}`);
        }
        ...
        if (jsonChunkType !== expectedJsonChunkType) {
            throw new TileFormatError_1.TileFormatError(`Expected chunk type to be ${expectedJsonChunkType}, but found ${jsonChunkType}`);
        }
        const jsonChunkStart = 20;
        const jsonChunkEnd = jsonChunkStart + jsonChunkLength;
        if (glbBuffer.length < jsonChunkEnd) {
            throw new TileFormatError_1.TileFormatError(`Expected at least ${jsonChunkEnd} bytes, but only got ${glbBuffer.length}`);
        }
        ...
        if (binChunkType !== expectedBinChunkType) {
            throw new TileFormatError_1.TileFormatError(`Expected chunk type to be ${expectedBinChunkType}, but found ${binChunkType}`);
        }
        const binChunkStart = binChunkHeaderStart + 8;
        const binChunkEnd = binChunkStart + binChunkLength;
        if (glbBuffer.length < binChunkEnd) {
            throw new TileFormatError_1.TileFormatError(`Expected at least ${binChunkEnd} bytes, but only got ${glbBuffer.length}`);
        }
    ...
    static async optimizeGlb(glbBuffer, options) {
        ...
        const customStage = async (gltf) => {
            await GltfPipelineLegacy_1.GltfPipelineLegacy.process(gltf);
            return gltf;
        };
