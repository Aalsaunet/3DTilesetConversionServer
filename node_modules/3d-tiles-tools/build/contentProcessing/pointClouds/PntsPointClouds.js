...
exports.PntsPointClouds = void 0;
const Iterables_1 = require("../../base/Iterables");
const DefaultPointCloud_1 = require("./DefaultPointCloud");
const TileTableData_1 = require("../../tileTableData/TileTableData");
const BatchTables_1 = require("../../tileTableData/BatchTables");
const Colors_1 = require("../../tileTableData/Colors");
const TileTableDataPnts_1 = require("../../tileTableData/TileTableDataPnts");
const DracoDecoder_1 = require("../draco/DracoDecoder");
/**
 * Methods to create `ReadablePointCloud` instances from PNTS data
 *
 * @internal
 */
class PntsPointClouds {
    /**
     ...
     */
    static async create(featureTable, featureTableBinary, batchTable) {
        const pointCloud = new DefaultPointCloud_1.DefaultPointCloud();
        ...
        const dracoDecoderResult = await PntsPointClouds.obtainDracoDecodedAttributes(featureTable, featureTableBinary, batchTable);
        if (dracoDecoderResult) {
            PntsPointClouds.assignDracoDecodedAttributes(pointCloud, numPoints, dracoDecoderResult, batchTable);
        }
        ...
        if (!pointCloud.getAttributeValues("POSITION")) {
            const positions = TileTableData_1.TileTableData.createPositions(featureTable, featureTableBinary, numPoints);
            pointCloud.setPositions(positions);
        }
        // Assign the normals
        if (!pointCloud.getAttributeValues("NORMAL")) {
            const normals = TileTableDataPnts_1.TileTableDataPnts.createNormals(featureTable, featureTableBinary, numPoints);
            if (normals) {
        ...
        if (!pointCloud.getAttributeValues("COLOR_0")) {
            const colors = TileTableDataPnts_1.TileTableDataPnts.createNormalizedLinearColors(featureTable, featureTableBinary, numPoints);
            if (colors) {
        ...
        if (!pointCloud.getAttributeValues("_FEATURE_ID_0")) {
            const batchIds = TileTableDataPnts_1.TileTableDataPnts.createBatchIds(featureTable, featureTableBinary);
            if (batchIds) {
                const componentType = TileTableData_1.TileTableData.obtainBatchIdComponentType(featureTable);
                if (componentType) {
        ...
        if (!pointCloud.getAttributeValues("COLOR_0")) {
            ...
            const globalColor = TileTableDataPnts_1.TileTableDataPnts.createGlobalNormalizedLinearColor(featureTable, featureTableBinary);
        ...
        const globalPosition = TileTableData_1.TileTableData.obtainGlobalPosition(featureTable, featureTableBinary);
     ...
     */
    static mayRequireAlpha(featureTable) {
     ...
     */
    static hasQuantizedPositions(featureTable) {
     ...
     */
    static hasOctEncodedNormals(featureTable) {
     ...
     */
    static async obtainDracoDecodedAttributes(featureTable, featureTableBinary, batchTable) {
        if (!featureTable.extensions) {
            return undefined;
        }
        const featureTableExtension = featureTable.extensions["3DTILES_draco_point_compression"];
        ...
        Object.assign(allProperties, BatchTables_1.BatchTables.obtainDracoProperties(batchTable));
        const dracoDecoder = await DracoDecoder_1.DracoDecoder.create();
     ...
     * @param batchTable - The batch table
     */
    static assignDracoDecodedAttributes(pointCloud, numPoints, dracoDecoderResult, batchTable) {
        // Assign the positions, if present
        const dracoPositions = dracoDecoderResult["POSITION"];
        if (dracoPositions) {
            const positions = TileTableData_1.TileTableData.createPositionsFromBinary(dracoPositions.attributeData, dracoPositions.attributeInfo.byteOffset, numPoints);
            pointCloud.setPositions(positions);
        }
        // Assign the normals, if present
        const dracoNormals = dracoDecoderResult["NORMAL"];
        if (dracoNormals) {
            const normals = TileTableDataPnts_1.TileTableDataPnts.createNormalsFromBinary(dracoNormals.attributeData, dracoNormals.attributeInfo.byteOffset, numPoints);
            pointCloud.setNormals(normals);
        }
        // Assign the colors, from RGB or RGBA data, if present
        const dracoRGB = dracoDecoderResult["RGB"];
        if (dracoRGB) {
            const colorsStandardRGB = TileTableDataPnts_1.TileTableDataPnts.createColorsStandardRGBFromBinary(dracoRGB.attributeData, dracoRGB.attributeInfo.byteOffset, numPoints);
            const colorsNormalizedLinearRGBA = Iterables_1.Iterables.map(colorsStandardRGB, Colors_1.Colors.standardRGBToNormalizedLinearRGBA);
            pointCloud.setNormalizedLinearColors(colorsNormalizedLinearRGBA);
        }
        const dracoRGBA = dracoDecoderResult["RGBA"];
        if (dracoRGBA) {
            const colorsStandardRGBA = TileTableDataPnts_1.TileTableDataPnts.createColorsStandardRGBAFromBinary(dracoRGBA.attributeData, dracoRGBA.attributeInfo.byteOffset, numPoints);
            const colorsNormalizedLinearRGBA = Iterables_1.Iterables.map(colorsStandardRGBA, Colors_1.Colors.standardRGBAToNormalizedLinearRGBA);
            pointCloud.setNormalizedLinearColors(colorsNormalizedLinearRGBA);
        }
        ...
        if (dracoBatchId) {
            ...
            const batchIds = TileTableData_1.TileTableData.createBatchIdsFromBinary(dracoBatchId.attributeData, dracoBatchId.attributeInfo.byteOffset, legacyComponentType, numPoints);
            const componentType = TileTableData_1.TileTableData.convertLegacyComponentTypeToComponentType(legacyComponentType);
            pointCloud.addAttribute("_FEATURE_ID_0", "SCALAR", componentType, batchIds);
        }
        ...
        const dracoPropertyNames = BatchTables_1.BatchTables.obtainDracoPropertyNames(batchTable);
        for (const propertyName of dracoPropertyNames) {
            ...
            if (dracoProperty) {
                ...
                if (TileTableData_1.TileTableData.isBatchTableBinaryBodyReference(propertyValue)) {
                    const legacyType = propertyValue.type;
                    const legacyComponentType = dracoProperty.attributeInfo.componentDatatype;
                    const prpertyValues = TileTableData_1.TileTableData.createNumericScalarIterable(legacyType, legacyComponentType, dracoProperty.attributeData, dracoProperty.attributeInfo.byteOffset, numPoints);
                    const type = TileTableData_1.TileTableData.convertLegacyTypeToType(legacyType);
                    const componentType = TileTableData_1.TileTableData.convertLegacyComponentTypeToComponentType(legacyComponentType);
                    pointCloud.addAttribute(propertyName, type, componentType, prpertyValues);
                }
            }
        }
    }
}
exports.PntsPointClouds = PntsPointClouds;
