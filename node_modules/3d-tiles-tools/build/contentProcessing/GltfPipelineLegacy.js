...
class GltfPipelineLegacy {
    ...
    static addCesiumRTC(gltf, options) {
        options = (0, defaultValue_1.defaultValue)(options, {});
        const positionArray = [];
        const ellipsoid = (0, defaultValue_1.defaultValue)(options.ellipsoid, cesium_1.Ellipsoid.WGS84);
        let position = options.position;
        if (!(0, cesium_1.defined)(position)) {
            if ((0, cesium_1.defined)(options.longitude) &&
                (0, cesium_1.defined)(options.latitude) &&
                (0, cesium_1.defined)(options.height)) {
                position = cesium_1.Cartesian3.fromRadians(options.longitude, options.latitude, options.height, ellipsoid);
            }
            else {
                throw new cesium_1.DeveloperError("Either a position or lat/long/height must be provided");
            }
        }
        cesium_1.Cartesian3.pack(position, positionArray);
        let extensions = gltf.extensions;
        if (!(0, cesium_1.defined)(extensions)) {
            extensions = {};
        ...
        Extensions_1.Extensions.addExtensionRequired(gltf, "CESIUM_RTC");
    }
